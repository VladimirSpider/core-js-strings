...
function isPalindrome(str) {
  // throw new Error('Not implemented');
  const baseStr = str.replaceAll(/[\s?!,]/g, '').toLowerCase();
  const newStr = baseStr.split('').reverse().join('');

  return baseStr === newStr;
}
...
function reverseWords(str) {
  ...
  arrWords.forEach((word, index) => {
    arrWords[index] = word.split('').reverse().join('');
  });
...
function invertCase(str) {
  ...
  letters.forEach((letter, index) => {
    if (bigLetters.test(letter)) letters[index] = letter.toLowerCase();
    if (smallLetters.test(letter)) letters[index] = letter.toUpperCase();
  });
...
function encodeToRot13(str) {
  ...
  const currentLetters = str.split('');
  const bigLettersRange = /[A-Z]/;
  const smallLettersRange = /[a-z]/;
  const code = [];
  currentLetters.forEach((letter) => {
    if (bigLettersRange.test(letter)) {
      let index = bigLetters.indexOf(letter);
      let i = 0;
      while (i < 13) {
        i += 1;
        index += 1;
      ...
      code.push(bigLetters[index]);
    } else if (smallLettersRange.test(letter)) {
      let index = smallLetters.indexOf(letter);
      let i = 0;
      while (i < 13) {
        i += 1;
        index += 1;
